...
def _trim_white_margin(image: Image.Image, margin: int) -> Image.Image:
    if margin <= 0:
        return image.copy()
    ...
    if not bbox:
        return image.copy()
    left = max(0, min(bbox[0], margin))
    top = max(0, min(bbox[1], margin))
    right = min(image.width, max(bbox[2], image.width - margin))
    bottom = min(image.height, max(bbox[3], image.height - margin))
...
def restore_artwork(
    ...
    _trim_white_margin(artwork, trim_margin).save(
