...
import { AreaMap, areaStage, stageLabels, type MapArea } from "./AreaMap.js";

type Area = MapArea & { id: number; level: "state" | "district" };
...
export function AreasPage({ api }: ModulePageProps) {
  ...
  const rows = useMemo(() => (areas.data ?? []).filter((area) => {
    if (area.level !== "district") return false;
    if (state !== "all" && area.stateName !== state) return false;
    if (selectedAgs !== null && area.ags !== selectedAgs) return false;
  ...
  }), [areas.data, filter, state, selectedAgs, now]);
  ...
  return <div className="stack">
    ...
    <Card><AreaMap
      ...
      onSelectDistrict={setSelectedAgs}
      onSelectState={(value) => { setState(value); setSelectedAgs(null); }}
    /></Card>
    ...
    </select></label>{selectedAgs !== null && <Button onClick={() => setSelectedAgs(null)}>Gebietsauswahl aufheben</Button>}{capabilities.data?.run && <Button onClick={() => void run()}>Fällige Gebiete suchen</Button>}</Card>
    <Card><div className="table-wrap"><table><thead><tr><th>Bundesland</th><th>Gebiet</th><th>Stand</th><th>Letzter Lauf</th><th>Nächste Fälligkeit</th><th>Funde</th></tr></thead>
      <tbody>{rows.map((area) => <tr key={area.id}><td>{area.stateName}</td><td>{area.name}</td><td>{stageLabels[areaStage(area, now)]}</td><td>{date(area.lastRunAt)}</td><td>{date(area.nextDueAt)}</td><td>{area.foundSources}</td></tr>)}</tbody>
    </table></div></Card>
  </div>;
}
